...
import { BestSellers, GoogleBook } from "../types";
...
export const getBestSellers = async (): Promise<BestSellers[]> => {
    try {
        const response = await axios.get(`${bestSellersUrl}overview.json?api-key=${nyt_key}`);
        
        const bestSellerBooks:BestSellers[] = response.data.results.lists[0].books.map((item) => ({
            title: item.title,
            author: item.author,
...
export const searchBooks = async(query: string): Promise<GoogleBook[]> => {
    try {
        const response = await axios.get(`${googleBooksUrl}?q=${query}&api-key=${google_key}`)
